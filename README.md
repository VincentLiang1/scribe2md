...
3. 雙擊桌面上的「**AI 文件.MD 轉換器**」(或工具資料夾裡的 **`啟動.bat`**)——瀏覽器會自動開啟工具頁面。啟動時會先閃出一個黑色視窗,**網頁開好之後它會自己隱藏**,不必理會它
...
- 會先出現一個黑色文字視窗,幾秒後**瀏覽器自動打開工具頁面**,那個視窗接著就**自己隱藏起來**了——它還在跑,只是不再佔著你的畫面。
- **用完把瀏覽器分頁關掉就好**,工具會自己結束(轉檔或錄音還在跑時它會等跑完)。
- 瀏覽器如果沒有自動打開,黑色視窗**會留著不隱藏**(這是刻意的:那時它顯示的網址是你唯一的線索)。自己開瀏覽器,照那個視窗上的網址輸入,通常是 `127.0.0.1:7860`——若 7860 被占用,工具會自動改用下一個號碼。
...
**Q:瀏覽器沒有自動打開?** 這種時候黑色視窗**會留著不隱藏**(刻意的:那個網址是你唯一的線索)。自己開瀏覽器,輸入那個視窗上顯示的網址,通常是 `127.0.0.1:7860`;號碼被其他程式占用時會自動遞增。順帶一提,平常網頁開得起來時那個視窗會自己藏起來,**這不是關掉了**。
