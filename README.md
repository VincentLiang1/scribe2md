...
把**文字、圖像、聲音**都轉成**適合 AI 閱讀的 Markdown 檔案格式**:會議錄音或錄影變成「**誰說了什麼**」的**繁體中文逐字稿**,Word、PowerPoint、Excel、PDF、掃描件等文件轉成 Markdown。所有處理都在本機電腦完成,**錄音檔、文件和逐字稿都不會上傳到任何地方**。(只有兩種情況會連網:第一次使用時下載 AI 模型、以及轉**舊版** Office 檔而電腦上沒有 LibreOffice 時下載它——舊版指副檔名**結尾沒有 x** 的 `doc`/`xls`/`ppt`,現在的 `docx`/`xlsx`/`pptx` 不會下載任何東西。)
