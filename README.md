...
1. 解壓縮 zip 到桌面或 `C:\` 下的英文資料夾
2. 雙擊 **`安裝.bat`**——若跳出藍色「Windows 已保護您的電腦」,點「其他資訊」→「仍要執行」(這是正常的);黑色視窗自動安裝 3～10 分鐘,看到 `環境建置完成` 就是成功
3. 雙擊 **`啟動.bat`**——瀏覽器會自動開啟工具頁面;**黑色視窗要保持開著**(關掉工具就停了)
...
4. 「講者人數」就在左欄主畫面上,平常留 **0**(自動偵測)就好。其餘設定(模型、CPU 核心數)都收在「**進階參數設定**」裡,用**預設值**就好
...
對 zip 檔按右鍵 →「全部解壓縮」。建議解壓到**桌面**或 `C:\` 底下的**英文名稱資料夾**(例如 `C:\tools`)。解壓後會得到一個 `meeting-scribe` 資料夾,裡面有 `安裝.bat`、`啟動.bat`、這份使用手冊(`README.md`)和一些程式檔案。
...
| ❓ 使用說明 | **每一個功能的完整說明**,依「你現在想做什麼」分成九篇 |
...
**Q:速度多快?** 較新的電腦(例如 Intel Core Ultra 7 + Arc 內建顯示晶片)會自動用內建 GPU 加速轉錄,同時用 CPU 做講者分析——兩件事並行,不互相等待。沒有可用顯示晶片的電腦上,兩個步驟會改為依序執行(避免互搶處理器),速度較慢屬正常。快速模型下 1 小時會議粗估 20～40 分鐘。**在有 GPU 的電腦上,「精準」和「快速」的總時間其實差不多**——因為講者分析(CPU)才是瓶頸、又和轉錄並行,精準多花的轉錄時間被藏在裡面(所以這類機器預設就用精準);只有在沒 GPU 的電腦上,精準才會明顯慢約 4 倍。處理時間大約和錄音**長度**成正比(例如 3 小時錄音約 1～2 小時),和檔案大小(MB 數)無關——幾百 MB 的大檔案不用擔心。處理期間可以把視窗放著,先去做別的事。工具頁面頂端會顯示偵測到的執行裝置(Intel GPU / NVIDIA GPU / CPU),完成後逐字稿預覽開頭則標示實際使用的裝置。另外:一口氣講很久、完全沒有停頓的段落,進度條會停在原地一陣子再一次前進一大段——黑色視窗有活動就是正常,不是當機。

一個已知例外要特別注意:**部分老舊電腦的內建顯示晶片(例如 Intel UHD 600 系列)也會被工具自動當成加速晶片,但它們太弱,實際可能比純 CPU 還慢——而且工具不會自動發現「太慢」而換回 CPU**。判斷方式:頁面頂端顯示「Intel GPU」、黑色視窗有活動、也不是在下載模型,但速度明顯不合理地慢。較新的機器不受影響;在老舊電腦上遇到這種情況,請聯絡把工具給你的人,或到 GitHub 開 issue 回報。
...
2. **系統需求相同**:Windows 10/11(x64)、記憶體 8GB 以上、硬碟約 8GB、第一次安裝與下載模型需要網路(見第五章)。不支援 macOS 和 Linux。
3. **速度依機器差很多**:沒有顯示晶片的舊電腦一樣能用,只是比較慢;速度預期與老舊內建顯示晶片的特別注意事項,見上面「速度多快?」那一題。

**Q:講者分得不準?** 開會前先數好人數,在「講者人數」填入確切數字會準很多。收音品質(離麥克風距離、環境噪音)也直接影響效果。關於「講者人數」留 0(自動偵測):工具會保留發言量足夠、且聲音與他人明顯不同的講者,只把「極短的雜訊碎段」和「同一人音量/距離變化造成的漂移」併掉。不過對**與會人數多(十幾人以上)、且很多人只講幾句**的大型會議,自動偵測仍可能少算幾位——當多人聲音相近、發言又短時,聲紋本身就難以分辨。**這種大型會議,直接在「講者人數」填入大概人數(最多 30)會比自動判斷可靠得多。**

**Q:怎麼更新新版本?** 把新版 zip 解壓縮、**覆蓋**到你現在的工具資料夾(檔案總管問「要取代嗎」就選取代),再雙擊一次 `安裝.bat` 就好。跟第一次安裝是同一個 zip,不必挑版本。
...
- 硬碟空間:約 8GB(執行環境 + 模型;若「快速」「精準」兩種模型都下載,再多抓 2～3GB)。若要轉舊版 Office 檔(`doc`/`ppt`/`xls`)且電腦上沒有 LibreOffice,會再多用約 1GB
