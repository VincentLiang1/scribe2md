...
一個已知例外要特別注意:**部分老舊電腦的內建顯示晶片(例如 Intel UHD 600 系列)也會被工具自動當成加速晶片,但它們太弱,實際可能比純 CPU 還慢——而且工具不會自動發現「太慢」而換回 CPU**。判斷方式:「❓ 使用說明」→「⚙️ 調準確度與速度」那篇的「本機偵測結果」寫著「Intel GPU」(轉完一份之後,逐字稿預覽開頭也會標),而且進度確實在動、也不是在下載模型,但速度明顯不合理地慢。較新的機器不受影響;在老舊電腦上遇到這種情況,請聯絡把工具給你的人,或到 GitHub 開 issue 回報。
