...
def mmss(t: float) -> str:
    """秒數 → 「H:MM:SS」/「MM:SS」顯示(錄音狀態列、關閉確認、即時預覽、核對表與跳針
    標記共用)。

    ⚠️ **介面上要顯示時長就叫這支,不要就地 `// 60`**:原生版移植時三處各自寫成
    「分:秒」,分鐘不進位,三小時的會議顯示成「已錄 187:44」(2026-09-16 使用者回報)。"""
...
def _run_transcribe(wav: Path, model_key: str, progress):
    ...
    **為什麼要子行程**(使用者 2026-08-07 選定):轉檔要降到 below-normal
    好讓「電腦還能用」,但介面跟轉錄在同一個行程裡,於是介面也一起被降
    ——滿載時停止鈕 15~35 秒才有反應,使用者等不到就以為程式死了。搬進
    子行程後只降子行程,父行程的視窗隨時回應。
...
def run_pipeline(
    ...
    # 主行程就是視窗本體,降它等於連介面一起降:滿載時停止鈕
    # 15~35 秒才有反應(暖機期 95 秒),使用者等不到就以為程式死了,
