...
def _default_mic():
    ...
    if mic is None:
        ...
        # 同上:HRESULT 只進記錄檔。在此之前這條路徑**什麼都沒留下**
        # ——2026-08-05 那次的記錄檔從轉檔完成到使用者重開程式之間一行都
        # 沒有,完全無從判斷是不是裝置問題。
        # ⚠️ **一行、不附堆疊**(2026-09-20 改):沒有麥克風的電腦(虛擬機、
        # 桌機沒插麥克風)按下錄音就是走這條,而那是**預期內**的狀況——畫面上
        # 已經以繁中講清楚了,記錄檔再噴 12 行 soundcard 內部堆疊只會讓人以為
        # 程式壞了。要診斷的資訊只有 HRESULT 那一個數字,`%r` 連型別一起留住。
        logger.debug("查詢預設麥克風失敗:%r", cause)
        raise UserFacingError(
