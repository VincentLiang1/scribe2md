...
**成立前提**:程式與使用者在**同一台機器**上——使用者看到的檔案總管路徑,
程式直接開得起來,原生選檔對話框也才有意義。2026-09-05 之前這個前提是靠
「只綁 127.0.0.1」撐著的(那時是本機網頁),現在它是桌面程式、前提本來就
成立。⚠️ 這個前提一旦改變(真的做成 Server),本模組整個不成立。

分工:`clean_path`/`validate` 是純把關(不依賴任何 UI,錯誤走
`UserFacingError` 交由 UI 層原樣顯示,spec §8);`pick_file` 開 Windows
原生對話框,只取路徑、檔案不複製。
"""
import threading
