...
**為什麼要這個**(使用者 2026-08-08 實際踩到,當時還是 gradio 版):轉檔
進度原本只活在 `gr.Progress` 裡,而它綁在**那一次事件執行**上——瀏覽器
把背景分頁的計時器節流、前端判定成「連線可能已中斷」、使用者照著提示
按下重新整理,新頁面就再也接不回那條進度。畫面回到剛開啟的樣子:檔名
沒了、進度沒了,**連停止鈕都沒了**——而轉檔其實還在後端跑。想中止只能
關掉程式,等於整場重來(當次是一支 3 小時 59 分的月會錄音,已經轉了
43 分鐘)。

⚠️ **原生視窗沒有那個斷線情境,但這個模組不是殘骸**:進度的真相集中在
一個地方、由 `pipeline` 單向回報,仍然是對的形狀——而下面那條心跳 log
現在是它更主要的職責。
...
設計同 `cancel.py`——模組級單例,不依賴任何 UI 模組。`pipeline` 只管
回報,要不要畫、畫成什麼樣是 `desktop.py` 的事。

**心跳 log 也由這裡發**(CLAUDE.md:任何可能跑超過一分鐘的迴圈都要有
心跳,否則沉默會被當成當機)。講者分析是全流程最慢的一段(4 小時錄音
要跑 1~3 小時),先前從頭到尾只印一行「子行程已就緒」——使用者唯一的
判斷依據只剩「電腦順不順」,而那跟「轉檔有沒有在跑」根本是兩回事。
⚠️ **沒有黑視窗之後這條更重要**:心跳現在只進記錄檔,而那是事後查
「它到底有沒有在動」唯一的憑據。進度資料本來就會流經這裡,順手印出來
...
# 是拿 259 份執行記錄回頭算出來的**:把每一次「預估還要 N 分」跟它之後真正
...
def _sample_locked(now: float) -> None:
    ...
    if done < samples[-1][1]:
        ...
        # 速率算成負的,ETA 就此消失一大段(拿真實記錄重放時抓到:某場
