...
#
# ⚠️ **2026-09-18 起試用 150**(使用者裁定要縮短散會後的等待,並指定「試用
# 幾天,真的不行再改回 5 分鐘」)。與 `live._CHUNK_SEC` 是一組的,兩顆要一起
# 改。實測(89 分鐘會議重播,`scripts/bench_live_tail.py`):停止時要補的尾巴
# 從 215 秒降到 65 秒、補算 22.4 → 6.8 秒;每塊 33 秒 → 16 秒(那就是「剛好在
# 算時按停止」要多等的上限);錄音中的 CPU 多 13%(重疊佔比 10% → 20%)。
#
# ⚠️ **已知代價,回頭判斷「要不要改回去」時就看這一條**:塊界變了,分群結果
# 就跟著變。五場真實會議與當天的生產結果相比,**沒有一場比較好**,方向一致
# 地略差:與使用者命名確認過的逐字稿對照,「一群裡混進別人」四場全部退步
# (−0.6~−3.4 個百分點)、「同一個人被拆散」持平或略退;未知比例五場有四場
# 上升;一場 2 人對談還多切出一群(同一個人要命名兩次)。⚠️ 那份對照偏向
# 生產結果(名字當初掛在 300 秒塊分出來的群上),所以單場不能定罪——但方向
# 一致,而且**門檻(_AUTO_STOP_SIM 那個窄窗)當初是拿 300 秒塊校準的**。
# 症狀會長成「同一個人出現兩列」或「未知變多」,真的遇到就把兩顆改回去。
_LIVE_CHUNK_SEC = 150
...
# ⚠️ **重聚只值 3%,不是 10%**(2026-08-15 從執行記錄實測):一場 3 小時
...
def _segment_and_embed(
    ...
    259 份執行記錄回頭查出來的):末塊通常遠短於一整塊——1084 秒的錄音
...
def voiceprints_for_spans(
    ...
    「重設講者」用(`desktop.App._relabel_work`):現成逐字稿裡已經有講者標籤與時間戳
