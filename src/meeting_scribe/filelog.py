...
- **檔頭記程式版本**:三週後拿一份 log 出來分析,沒有這行就只能從訊息長相
  ...
  沒有 `.git`**,所以另外吃套件版號與「打包.bat」戳的 `VERSION`(見
...
def _stamped() -> str:
    """交付版:「打包.bat」在打包當下戳進去的 `VERSION`(格式 `<sha> <日期>`)。

    **為什麼光有套件版號不夠**:交付版沒有 `.git`,而兩次 Release 之間本來
    就會重打包 zip 給同仁——只靠版號的話那幾份長得一模一樣,回報問題時分
    不出是哪一份。⚠️ **檔案內容必須是純 ASCII**:寫它的是 cp950 的
    「打包.bat」,中文一律由這裡補上,不從那邊寫進去。"""
    try:
