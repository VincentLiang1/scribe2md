...
# 進度的第二個出口。⚠️ **原生視窗沒有黑視窗**(`啟動.vbs` 用 pythonw 起,主控台
# 整個藏起來),而模型第一次下載是 2~3 GB——只 `print` 的話那段進度**掉在地上**,
# 使用者看到的是一個十幾二十分鐘不動的畫面,合理地判斷成當掉了。
# ⚠️ **這是加一個出口、不是換掉 print**:除錯用的「啟動DEBUG.bat」與命令列那條路
# 靠的正是主控台那一份。
# ⚠️ **會從工作執行緒被呼叫**(下載跑在轉檔那條執行緒上),所以掛進來的東西
# **不准直接碰 UI**——要自己丟回主執行緒(見 `desktop.App._model_progress`)。
_PROGRESS_HOOK = None


def set_progress_hook(fn) -> None:
    """讓下載進度多一個出口(`None` = 取消)。理由見 `_PROGRESS_HOOK`。"""
...
def download(
    ...
    def _hook(blocks: int, block_size: int, total: int) -> None:
        ...
        line = (f"下載{what} {dest.name}:{pct}%"
                f"({min(done, total) / 1048576:.0f}/{total / 1048576:.0f} MB)")
        try:
            print(line, flush=True)  # noqa: T201 - 黑視窗的進度回饋
            if _PROGRESS_HOOK is not None:
                _PROGRESS_HOOK(line, pct / 100)
        except Exception:
