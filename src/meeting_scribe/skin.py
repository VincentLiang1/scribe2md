...
SKIN_FRAMES = (
    ...
    # segmented control 的狀態選擇(「收音情境」與對話框那兩排,2026-09-15 使用者選案 C):
    # 灰槽,與槽裡選中那格的白膠囊(帶淡影)。⚠️ **是 Frame 不是 Button**——每一格裡面要放
    # 圖示與文字兩個 Label(它們的基線不同,理由見 `desktop.App._icon_pad`),而 ttk 的
    # Button 只吃得下一個 image + 一段文字。
    # ⚠️ **兩張都是膠囊,圖高必須精確等於元件高度**,所以用它們的 Frame 一定要 `height=`
    # 釘死並關掉傳播(見 `desktop.App._segmented`);漏掉就是垂直重複貼或被裁,兩種
    # 都不報錯、`reqheight` 也看不出來,只有截圖看得到。
    # ⚠️ **「要做什麼」那一排的槽(`Seg.TFrame`)不在這裡**:那一排長得跟分頁一樣、沒有
    # 底板,底色由 `desktop.configure_styles` 直接設成 `card`——這份名單是「誰的底色由共用
    # 包設」的來源,把它放進來就是要一張它不穿的圖。
    ("SegTrack.TFrame", "Sq.seg", "btn"),
    ...
    # 「要做什麼」那一排選中的那一格(2026-09-15 使用者選案 C:長得跟分頁一樣)。⚠️ **不可以
    # 直接穿 `SubOnCell.TFrame`**:那張圖的圓角外側畫的是視窗底,而這一排坐在白卡片上。
    ("SegTab.TFrame", "Sq.segtab", "btn"),
