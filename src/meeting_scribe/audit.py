...
def cut_one(src: Path, row: AuditRow, dest: Path) -> Path:
    ...
    **為什麼不是在整批音檔上定位播放**:核對表上要點的就是「這一段再聽一次」,
    而各剪一小段最直接、成本近乎零(隨機存取,實測 0.01 秒)。
    (先前這段的理由寫的是「gradio 的播放器沒有跳到第 N 秒的伺服器端 API」;
    那套介面 2026-09-05 移除了,而**結論沒變**——原生視窗那邊也是逐列播放,
    使用者 2026-08-13 指定的就是這個形狀。)"""
